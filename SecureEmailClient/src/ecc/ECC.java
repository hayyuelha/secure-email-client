...
public class ECC {
    ...
    public static PairRS createSignature(byte[] plainText, PrivateKey key) throws Exception {
        ...
        ECPoint res = c.multiply(g, k);
        
        BigInteger r = res.x.mod(n);
    ...
    public static boolean verify(byte[] plainText, PublicKey key,PairRS rs) throws Exception {
        ...
        if(!(rs.r.compareTo(BigInteger.ONE)>=0&&rs.r.compareTo(n)<=0&&rs.s.compareTo(BigInteger.ONE)>=0&&rs.s.compareTo(n)<=0)){
            return false;
        }
        ...
        BigInteger w = rs.s.modInverse(n);
        
        BigInteger u1 = e.multiply(w).mod(n);
        
        BigInteger u2 = rs.r.multiply(w).mod(n);
        
        ECPoint res = c.add(c.multiply(g, u1), c.multiply(publicKey, u2));
        
        return (res.x.compareTo(rs.r.mod(n))==0);
